...
func form_roll():
	if is_roll == true:
		animations.play("anim_halofire_roll_" + last_dir)
...
func form_melee():
	if is_melee == true:
		...
		animations.play("anim_halofire_slash_" + last_dir)
		await animations.animation_finished
		weapon.disable()
...
func form_special():
	if is_special == true:
		...
		animations.play("anim_halofire_special_" + last_dir)
		await animations.animation_finished
		player.is_attack = false
...
func update_animation():
	...
	if is_magic == true:
		...
		if animations:
			if _pVel.length() != 0:
				animations.play("anim_halofire_runCast_" + magic_dir)
			else:
				animations.play("anim_halofire_idleCast_" + last_dir)
	...
	if is_attack == false:
		if _pVel.length() != 0:
			...
			if _pVel.x < 0: 
				...
				direction = "up"
				last_dir = "up"
			animations.play("anim_halofire_walk_" + direction)
		else:
			animations.play("anim_halofire_idle_" + last_dir)
